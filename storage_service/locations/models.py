...
class Package(models.Model):
    ...
    def _store_aip_lom(self, file_path):
        ...
        relative_mets_path = os.path.join(
            os.path.splitext(os.path.basename(self.current_path))[0],
            "data",
            'METS.{}.xml'.format(self.uuid))
        mets_path = self.extract_file(relative_mets_path)
        mets = etree.parse(mets_path)

        # Parse out name and description if found
        slug = str(self.uuid)
        title = os.path.basename(self.current_path)
        summary = 'AIP generated by Archivematica with uuid {}'.format(self.uuid)
        ...
        if isinstance(output_files, list):
            for index, file_path in enumerate(output_files):
                # TODO external URLs is relative to SS url, need absolute
                external_url = reverse('download_lockss', kwargs={'api_name': 'v1', 'resource_name': 'file', 'uuid': self.uuid, 'chunk_number': str(index).zfill(2)})
                entry.add_field('lom_content', external_url)
                entry.entry[-1].set('size', str(os.path.getsize(file_path)))
        else:  # Only one item
            external_url = reverse('download_request', kwargs={'api_name': 'v1', 'resource_name': 'file', 'uuid': self.uuid})
            entry.add_field('lom_content', external_url)
            entry.entry[-1].set('size', str(os.path.getsize(output_files)))
