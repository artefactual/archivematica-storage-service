...
def _validate(params, args_, kwargs_):
    ...
    for param_name, param_val in params.items():
        ...
        param_dflt = param_cfg.get('default', Undefined)
        if not param_type:
            continue
        ...
        if ((param_val is None) and
                ...
                (not param_val == param_dflt) and
                (not isinstance(None, param_type))):
            to_delete.append(param_name)
            continue
        if ((not isinstance(param_val, param_type)) and
                (not param_val == param_dflt)):
            raise ValidationError(
...
def get_method(openapi_spec, path, path_params, http_method, op_cfg):
    ...
    if tags:
        tag = tags[0]
        tag_cfg = [t for t in openapi_spec['tags'] if t['name'] == tag][0]
        namespace_description = tag_cfg.get('description')
    ...
    return locals()[method_name], method_name, namespace, namespace_description
    ...
    for path, cfg in openapi_spec['paths'].items():
        ...
        for http_method, op_cfg in cfg.items():
            if http_method in HTTP_METHODS:
                method, method_name, namespace, namespace_description = (
                    get_method(openapi_spec, path, path_params, http_method,
                               op_cfg))
                methods.setdefault(namespace, {})
                methods[namespace][method_name] = method
                methods[namespace]['__doc__'] = namespace_description
    return methods
...
def get_rsrc_cls_docstring(resource_name, current_doc):
    if current_doc:
        return current_doc
...
def metaprog_client_class():
    ...
    for namespace, rsrc_methods in namespaces_methods.items():
        ...
        rsrc_methods['__doc__'] = get_rsrc_cls_docstring(
            namespace, rsrc_methods['__doc__'])
