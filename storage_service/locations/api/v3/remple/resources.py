...
from .schemata import PaginatorSchema, ValidModelObject
...
class ReadonlyResources(object):
    ...
    def __init__(self, request, server_path='/api/0_1_0/'):
...
class Resources(ReadonlyResources):
    ...
    def preprocess_user_data(self, validated_user_data, schema):
        ...
        for field_name, field in schema_cls.fields.items():
            ...
            if field_cls_name == 'ForEach' and isinstance(
                    field.validators[0], ValidModelObject):
                # if value:
    ...
    def _create_new_resource(self, validated_user_data, schema):
        ...
        for attr, vals in m2m.items():
            try:
                getattr(ret, attr).clear()
            except ValueError:
                pass
            for val in vals:
                getattr(ret, attr).add(val)
        return ret
    ...
    def _update_resource_model(self, resource_model, validated_user_data,
        ...
        if changed:
            for attr, user_val in self.preprocess_user_data(
                ...
                existing_val_cls_name = existing_val.__class__.__name__
                through = getattr(existing_val, 'through', None)
                if (existing_val_cls_name == 'ManyRelatedManager' and
                        through and
                        (not through._meta.auto_created)):
                    this_attr = [f for f in through._meta.get_fields()
                                 if f.__class__.__name__ == 'ForeignKey'][0].name
    ...
    def _distinct(self, attr, resource_model, new_val):
        ...
        field_cls_name = field.__class__.__name__
        existing_val = getattr(resource_model, attr)
        if field_cls_name == 'ManyToManyField':
        ...
        field = resource_model.__class__._meta.get_field(attr)
        logger.info(field.__class__.__name__)

        ret =  new_val != existing_val

        if ret:
            logger.info('PIG value for {} is distinct!'.format(attr))
    ...
    @staticmethod
    def _get_related_model_getter(field):
        related_model_cls = field.model_cls
        def getter(model_cls):
            return [mi.uuid for mi in model_cls.objects.all()]
    ...
    @classmethod
    def _django_model_class_to_plural(cls, model_cls):
        return cls.inflect_p.plural(model_cls.__name__.lower())

    @classmethod
    def _get_new_edit_collections(cls, mode='new'):
        ...
        introspecting both ``cls.model_cls`` and ``cls.schema_cls``.
        """
        collections = {}
        if mode == 'new':
            schema_cls = cls.get_create_schema_cls()
        else:
            schema_cls = cls.get_update_schema_cls()
        for field_name, field in schema_cls.fields.items():
            field_cls_name = field.__class__.__name__
            if field_cls_name == 'ValidModelObject':
                key = cls._django_model_class_to_plural(field.model_cls)
                collections[key] = cls._get_related_model_getter(field)
            elif field_cls_name == 'ForEach':
                first_validator = field.validators[0]
                field_cls_name = first_validator.__class__.__name__
                if field_cls_name == 'ValidModelObject':
                    key = cls._django_model_class_to_plural(
                        first_validator.model_cls)
                    collections[key] = cls._get_related_model_getter(first_validator)
        return collections

    def to_dict(self, instance):
        opts = instance._meta
        data = {'resource_uri': '{}/{}/{}/'.format(
            self.server_path, self.collection_name, instance.uuid)}
