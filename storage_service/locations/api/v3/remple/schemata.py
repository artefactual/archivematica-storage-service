...
from formencode.validators import Int, FancyValidator, Regex

from .constants import formencode_field_class2openapi_type
...
class OpenAPISchema(object):
    ...
    def extract_parameters(self):
        ...
        for parameter_name, formencode_cls in (
            ...
            schema['type'] = formencode_field_class2openapi_type.get(
                formencode_cls.__class__.__name__, 'string')
...
class ValidModelObject(FancyValidator):
    ...
    usage: ValidModelObject(model_cls=models.Package).
    """

    messages = {
        'invalid_model':
        'There is no %(model_name_eng)s with pk %(pk)s.'
    }
    ...
    def _convert_to_python(self, value, state):
        if value in ['', None]:
            ...
            pk = pk_validator().to_python(value, state)
...
__all__ = ('schemata', 'PaginatorSchema', 'ValidModelObject')
