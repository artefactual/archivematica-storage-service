...
def collection(request, location):
    ...
    if isinstance(location, basestring):
        try:
            ...
            return helpers.sword_error_response(request, 404, _('Collection {uuid} does not exist.').format(uuid=location))
...
def deposit_edit(request, deposit):
    ...
    if isinstance(deposit, basestring):
        try:
            ...
            return helpers.sword_error_response(request, 404, _('Deposit location {uuid} does not exist.').format(uuid=deposit))
...
def deposit_state(request, deposit):
    ...
    if request.method == 'GET':
        ...
        state_term = status
        state_description = _('Deposit initiation: {status}').format(status=status)
...
def _handle_upload_request_with_potential_md5_checksum(request, file_path, success_status_code):
    ...
    if 'HTTP_CONTENT_MD5' in request.META:
        ...
        if request.META['HTTP_CONTENT_MD5'] != md5sum:
            ...
            return helpers.sword_error_response(request, 400, _('MD5 checksum of uploaded file ({uploaded_md5sum}) does not match checksum provided in header ({header_md5sum}).').format(
