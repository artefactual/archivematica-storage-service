...
class FileSerializer(serializers.HyperlinkedModelSerializer):
    ...
    """
    pipeline = serializers.ReadOnlyField(source='origin')

    class Meta:
        model = models.File
        fields = ('uuid', 'name', 'file_type', 'size', 'format_name',
                  'pronom_id', 'pipeline', 'source_package', 'normalized',
                  'valid', 'ingestion_time')
...
class FileFilter(django_filters.FilterSet):
    ...
    ingestion_time_at_or_before = django_filters.DateFilter(name='ingestion_time', lookup_type='lt')
    ingestion_time_at_or_after = django_filters.DateFilter(name='ingestion_time', lookup_type='gt')

    class Meta:
        model = models.File
        fields = ['uuid', 'name', 'file_type', 'min_size', 'max_size',
                  'format_name', 'pronom_id', 'pipeline', 'source_package',
                  'normalized', 'valid', 'ingestion_time',
                  'ingestion_time_at_or_before', 'ingestion_time_at_or_after']
