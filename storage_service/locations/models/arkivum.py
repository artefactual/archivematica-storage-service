...
class Arkivum(models.Model):
    ...
    def is_file_local(self, package, path=None, email_nonlocal=False):
        ...
        if package_info.get('local'):
            ...
            if email_nonlocal:
                if path:
                    item = _('File {path} in package {package}').format(path=path, package=package)
                else:
                    item = _('Package {package}').format(package=package)
                message = _('{item} with Arkivum ID of {package_id} has been requested but is not available in the Arkivum cache.').format(
                    item=item,
