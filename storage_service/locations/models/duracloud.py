...
class Duracloud(models.Model):
    ...
    def _download_file(self, url, download_path, expected_size=0, checksum=None, retry=0):
        """
        ...
        if expected_size and os.path.getsize(download_path) != expected_size:
            if retry < 3:
                LOGGER.error("[RETRY=%(retry)d] File %(path)s does not match expected size of %(expected_size)s bytes, but was actually %(actual_size)s bytes" % {
                    "retry": retry,
                    "path": download_path,
                    "expected_size": expected_size,
                    "actual_size": os.path.getsize(download_path)
                    })
                return self._download_file(url, download_path, expected_size=expected_size, checksum=checksum, retry=retry + 1)
            else:
                raise StorageException(
                    _(
