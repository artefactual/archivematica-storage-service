...
class Lockssomatic(models.Model):
    ...
    au_size = models.BigIntegerField(verbose_name=_("AU Size"), null=True, blank=True,
        help_text=_("Size in bytes of an Allocation Unit"))
    sd_iri = models.URLField(max_length=256, verbose_name=_("Service Document IRI"),
        help_text=_("URL of LOCKSS-o-matic service document IRI, eg. http://lockssomatic.example.org/api/sword/2.0/sd-iri"))
    collection_iri = models.CharField(max_length=256, null=True, blank=True, verbose_name=_("Collection IRI"),
        help_text=_("URL to post the packages to, eg. http://lockssomatic.example.org/api/sword/2.0/col-iri/12"))
    ...
    external_domain = models.URLField(verbose_name=_('Externally available domain'),
        help_text=_('Base URL for this server that LOCKSS will be able to access.  Probably the URL for the home page of the Storage Service.'))
    checksum_type = models.CharField(max_length=64, null=True, blank=True, verbose_name=_('Checksum type'), help_text=_('Checksum type to send to LOCKSS-o-matic for verification.  Eg. md5, sha1, sha256'))
    keep_local = models.BooleanField(blank=True, default=True, verbose_name=_("Keep local copy?"),
        help_text=_("If checked, keep a local copy even after the AIP is stored in the LOCKSS network."))
