...
class TestDataverse(TestCase):
    ...
    def test_get_query_and_subtree(self):
        ...
        query_tests = [
            ...
            {"query": "query: drinks_4",
             "result": (None, None, "drinks_4")},

            {"query": "query: drinks_5/555",
             "result": ("555", None, "drinks_5")},
        ...
        for test in query_tests:
            # Retrieve our query our result sets.
            query_ = test.get("query")
            res_ = test.get("result")

            # Assign values to: Dataset, Subtree, Query
            d1, s1, q1 = self.dataverse.get_query_and_subtree(query_)
            d2, s2, q2 = res_
            assert d2 == d2
            assert s1 == s2
            assert q1 == q2
