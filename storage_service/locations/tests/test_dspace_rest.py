...
METS_1_EXTRACTABLE_METADATA = [
    {'language': '',
     'value': METS_1_DC_AUTHOR,
     'key': 'dcterms.author'},
    {'language': '',
     'value': METS_1_DC_IDENTIFIER,
     'key': 'dcterms.identifier'},
    {'language': '',
     'value': METS_1_DC_TITLE,
     'key': 'dc.title'}]
...
DS_REST_ITEM_CREATE_URL_TMPLT = (
    '{}/collections/'.format(DS_REST_URL) + '{}/items')
...
HEADER_SET_COOKIE_DFLT = (
    'JSESSIONID={};path=/rest;Secure;HttpOnly'.format(COOKIE))
COOKIES = {'JSESSIONID': COOKIE}
DS_POST_JSON_RESP_DFLT = {
    'uuid': DS_ITEM_UUID,
    'handle': DS_ITEM_HANDLE}
...
AIP_EXTRACTED_METS_RELATIVE_PATH = os.path.join(
    AIP_NAME, 'data', AIP_METS_FILENAME)
...
ASRequestValidity = namedtuple(
    'ASRequestValidity', 'is_valid method_that_raises exc')

as_valid = ASRequestValidity(
    is_valid=True,
    method_that_raises=None,
    exc=None)

as_constructor_exc = ASRequestValidity(
    is_valid=False,
    method_that_raises='constructor',
    exc=Exception('Could not login'))
...
class FakeDSpaceRESTPOSTResponse(object):
    ...
    def __init__(self, **kwargs):
        self.header_set_cookie = kwargs.get(
            'header_set_cookie', HEADER_SET_COOKIE_DFLT)
...
@pytest.mark.parametrize(
    ('package', 'ds_aip_collection', 'metadata', 'package_source_path',
     'package_mets_path', 'ds_request_validity', 'as_credentials_set',
    ...
    [
        ...
        MoveFromCaseAIP(package=MockPackage(fake_mets_file=open(METS_2_PATH)),
        ...
        # exception.
        MoveFromCaseDIP(as_credentials_set=True,
                        as_credentials_valid=as_constructor_exc),
        # Simulate ``client.add_digital_object(...) raising
        # ``CommunicationError``.
        MoveFromCaseDIP(as_credentials_set=True,
                        as_credentials_valid=as_ado_exc),
...
def test_move_from_storage_service(mocker, package, ds_aip_collection,
    ...
    if not package:
        ret = dspace_rest_space.move_from_storage_service(
    ...
    if ((not as_credentials_valid.is_valid) and
            as_credentials_valid.method_that_raises == 'constructor'):
        with pytest.raises(dspace_rest.DSpaceRESTException) as excinfo:
            dspace_rest_space.move_from_storage_service(
                package_source_path, AIP_DEST_PATH, package=package)
    ...
    if package.package_type == Package.DIP:
        ...
        if as_credentials_set:
            ...
            assert fake_as_client.args == (
                '/repositories/{}/archival_objects/'.format(AS_REPOSITORY),
                PACKAGE_UUID)
    ...
    # Assertions about the 2 subprocess.Popen calls:
    # 1. to unar to extract the AIP METS
    # 2. to dsmc (Tivoli Storage Manager)
    ((_, actual_unar_args, _),
     (_, actual_dsmc_args, _)) = subprocess.Popen.mock_calls
    assert actual_unar_args == ([
        'unar',
        '-force-overwrite',
        '-o',
        AIP_SOURCE_PATH_DIR,
