...
def test_command_fails_when_checksum_is_missing_for_compressed_aip(
    ...
    assert (
        captured.out.strip("\n")
        == "Complete. Checksums added for 0 of 1 identified AIPs. See output for details."
    )
...
def test_command_for_compressed_aip(
    get_compressed_package_checksum: mock.Mock,
    capsys: pytest.CaptureFixture[str],
    compressed_package: models.Package,
    aip_storage_fs_location: models.Location,
    ...
    algo = "sha256"
    get_compressed_package_checksum.return_value = (
        compressed_package_checksum,
        algo,
    )
    ...
    assert captured.out.split("\n")[:-1] == [
        ...
        "Complete. Checksums added for all 1 identified AIPs.",
    ]


@pytest.mark.django_db
@mock.patch("archivematica.storage_service.common.utils.generate_checksum")
def test_command_for_uncompressed_aip(
    generate_checksum: mock.Mock,
    capsys: pytest.CaptureFixture[str],
    uncompressed_package: models.Package,
    aip_storage_fs_location: models.Location,
) -> None:
    uncompressed_package_checksum = "098f6bcd4621d373cade4e832627b4f9"
...
@mock.patch("archivematica.storage_service.common.utils.generate_checksum")
def test_command_to_download_aips(
    generate_checksum: mock.Mock,
    get_compressed_package_checksum: mock.Mock,
    capsys: pytest.CaptureFixture[str],
    compressed_package: models.Package,
    ...
    total_packages = len(models.Package.objects.all())
    ...
    assert captured.out.split("\n")[:-1] == [
        ...
        f"Complete. Checksums added for all {total_packages} identified AIPs.",
    ]
...
def test_command_fails_when_checksum_is_missing_for_uncompressed_aips(
    ...
    assert captured.out.split("\n")[:-1] == [
        ...
        f"Complete. Checksums added for 1 of {total_packages} identified AIPs. See output for details.",
    ]
